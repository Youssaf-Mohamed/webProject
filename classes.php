...
abstract class User
{
    ...
    public static function login($email, $password)
    {
        ...
        if ($arr = mysqli_fetch_assoc($rslt)) {
            ...
            switch ($arr["role"]) {
                case 'subscriber':
                    $user = new Subscriber($arr["id"], $arr["name"], $arr["email"], $arr["phone"], $arr["image"], $arr["role"], $arr["created_at"], $arr["updated_at"]);

                    break;
                case 'admin':
                    $user = new Admin($arr["id"], $arr["name"], $arr["email"], $arr["phone"], $arr["image"], $arr["role"], $arr["created_at"], $arr["updated_at"]);
                    break;
            }
        }
